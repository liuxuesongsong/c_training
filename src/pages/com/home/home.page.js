...
class Home extends Component {
	...
	cacheToState() {
		...
		let enrolled = 0, arranged = 0, passed = 0, examing = 0,
		    enrolled_comp = 0,arranged_comp = 0,
			unarragedStudents = [], arrangedStudents = [],
			messages_pm=[],messages_comp=[],
			unarragedStudents_comp = [], arrangedStudents_comp = [];
		for (var i = 0; i < students.length; i++) {
		
			if (students[i].is_inlist == STATUS_ENROLLED_DID||students[i].is_inlist == STATUS_ARRANGED_DID) {
				if(students[i].course_id==1||students[i].course_id==2){
				enrolled++;
				unarragedStudents.push(students[i]);
				}else
				if(students[i].course_id==3||students[i].course_id==4){
					enrolled_comp++;
					unarragedStudents_comp.push(students[i]);
					}
			...
			if (students[i].is_inlist == 3) {
				...
				if(students[i].course_id==3||students[i].course_id==4){
					enrolled_comp++;
					arranged_comp++;
					arrangedStudents_comp.push(students[i]);
		...
		for(var i = 0; i < cancel_students.length; i++) {
			...
			if(students[i].course_id==3||students[i].course_id==4){
				unarragedStudents_comp.push(cancel_students[i]);
        ...
        for(var i=0;i<window.currentPage.state.messages.length;i++){
		//	console.log(window.currentPage.state.messages[i])
			if(window.currentPage.state.messages[i].course_id==1||window.currentPage.state.messages[i].course_id==2){
				messages_pm.push(window.currentPage.state.messages[i])
			}
			if(window.currentPage.state.messages[i].course_id==3||window.currentPage.state.messages[i].course_id==4){
				messages_comp.push(window.currentPage.state.messages[i])
			}
		}
		console.log(messages_pm)
		console.log(messages_comp)
		//console.log(window.currentPage.state.messages)
		// for(var i = 0; i<messages.length;i++){
		// 	this.state.messages.push(messages[i])
		// }
		//已安排占已报名的百分比
		var per = sessionStorage.classify==1?(arranged / enrolled * 288):(arranged_comp / enrolled_comp * 288);
		if (arranged != 0) {
		...
		window.currentPage.setState({
			name: name,
			enrolled: enrolled,
			arranged_comp: arranged_comp,
			enrolled_comp: enrolled_comp,
			arranged: arranged,
			examing: examing,
			passed: passed,
			...
			messages_pm:messages_pm,
			messages_comp:messages_comp,
...
<div style={{ paddingTop: "0" }} className={'nyx-paper'}>
      {/* 运维修改 */}
	   {sessionStorage.classify==1?
		...
		<span>
		{Lang[window.Lang].pages.com.home.arranged + ":"
		+ this.state.arranged_comp + Lang[window.Lang].pages.com.home.human}
		</span>
		<span style={{ float: "right" }}>
		{Lang[window.Lang].pages.com.home.unarranged + ":"
		+ (this.state.enrolled_comp-this.state.arranged_comp) + Lang[window.Lang].pages.com.home.human}
		</span>
		</div>
		<span className="nyx-arranged-per">
		...
		<span>
		{Lang[window.Lang].pages.com.home.enrolled + "共:"
		+ this.state.enrolled_comp + Lang[window.Lang].pages.com.home.human}
		</span>
		</div>
		</Paper>} 
	...
	{/* 运维修改 */}
		{sessionStorage.classify==1?this.state.messages_pm.map((message,index) => {
//{/* 运维修改 */}
...
case "5":
 return (
	...
	<div
	 style={{marginLeft:"1.2rem",height:"18px",marginTop:"2px"}}>
	 <p title={message.name} className="nyx-message-hidden" style={{width:"2.5rem"}}>{message.name}</p> 
		 ...
		 <p title={getCourse(message.course_id)+"培训班"} className="nyx-message-hidden" style={{width:"6.8rem",marginLeft:"0.2rem"}}>{getCourse(message.course_id)}培训班</p>
...
case "6":
 return (
   ...
   :this.state.messages_comp.map((message,index) => {
